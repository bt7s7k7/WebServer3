...
if (typeof require == "undefined" && typeof self != "undefined" && typeof process == "undefined") {
	...
	B.readAllData = function (stream, callback) {
		var buffers = []
		stream.on("data", (data) => {
			buffers.push(data)
		})
...
B.parseQuery = function (query, delim = "&", equals = "=") {
	if (!query) query = ""
	query = query.trim().replace(/\+/g,"%20")
	var ret = {}
	query.split(delim).map(v => {
		var p = v.split(equals)
		ret[p[0].trim()] = decodeURIComponent(p[1]).trim()
	})
	return ret
